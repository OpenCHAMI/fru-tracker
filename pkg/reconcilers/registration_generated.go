...
func RegisterReconcilers(controller *reconcile.Controller, client reconcile.ClientInterface, eventBus events.EventBus) error {
	// Register Device reconciler
	devicesReconciler := NewDefaultDeviceReconciler(client, eventBus)
	if err := controller.RegisterReconciler(devicesReconciler); err != nil {
		return err
	}
	// Register DiscoverySnapshot reconciler
	discoverysnapshotsReconciler := NewDefaultDiscoverySnapshotReconciler(client, eventBus)
	if err := controller.RegisterReconciler(discoverysnapshotsReconciler); err != nil {
		return err
	}
