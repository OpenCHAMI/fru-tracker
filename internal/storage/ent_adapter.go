...
import (
	...
	"github.com/example/fru-tracker/internal/storage/ent/annotation"
...
func FromEntResource(ctx context.Context, entResource *ent.Resource) (interface{}, error) {
	...
	case "DiscoverySnapshot":
		...
		// Unmarshal Spec
		if err := json.Unmarshal(entResource.Spec, &resource.Spec); err != nil {
			return nil, fmt.Errorf("failed to unmarshal spec for DiscoverySnapshot: %w", err)
		}

		// Unmarshal Status
		if len(entResource.Status) > 0 && string(entResource.Status) != "null" {
			if err := json.Unmarshal(entResource.Status, &resource.Status); err != nil {
				return nil, fmt.Errorf("failed to unmarshal status for DiscoverySnapshot: %w", err)
			}
		}
